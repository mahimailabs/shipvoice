...
It prints one line per check with the fix underneath. Without `--live` it reads
...
- It parses the **root** `.env` only. A key that lives in `agent/.env` and
  nowhere else is invisible to it, and it will report that key as unset.
...
## You edited the wrong `.env`

`docker compose` reads the root `.env` and nothing else. The per-service files
(`agent/.env`, `backend/.env`, `frontend/.env`) are for running that service by
hand.

The symptom is a stack that works one way and not the other: the manual run
talks, compose does not, or the reverse.
...
Two different refusals, and the status code tells them apart.
...
- **409.** The backend has no read-write mount of `./agent/prompts`, so the
  save cannot land. The message names the path it could not write, and the
  prompt the agent is running is unchanged.

See [Persona](/persona).

## Console edits to the LiveKit project do not reach the worker

The worker takes its LiveKit project from the backend when `BACKEND_API_URL`
and `BACKEND_API_TOKEN` are both set, and restarts itself when the stored
project changes. Two failures:

- `AGENT_SERVICE_TOKEN` and `BACKEND_API_TOKEN` differ. They are one secret
  with two names. The backend answers 403 and the worker logs that it was
  refused.
